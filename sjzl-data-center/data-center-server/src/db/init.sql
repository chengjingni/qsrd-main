...
CREATE DATABASE wru_datacenter DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
Use wru_datacenter;
...
`secret` VARCHAR(50) COMMENT '密钥',
`index_url` VARCHAR(100) COMMENT '首页地址',
...
CREATE TABLE MESSAGE_INFO (
  ID INT(11) NOT NULL AUTO_INCREMENT,
  MESSAGE_TYPE CHAR(1) DEFAULT '0' COMMENT '消息类型（0待办事项/1公告/2通知/3新闻）',
  TITLE VARCHAR(300) DEFAULT NULL COMMENT '标题',
  CONTENT VARCHAR(4000) DEFAULT NULL COMMENT '消息内容（模板内容/手动编辑内容）',
  SENDER VARCHAR(100) DEFAULT NULL COMMENT '发送方/应用编号',
  RECEIVER VARCHAR(100) DEFAULT NULL COMMENT '接收方（学生/老师/所有人）',
  SEND_TIME TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP COMMENT '发送时间',
  RECEIVE_TIME TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP COMMENT '接收时间',
  READ_STATUS CHAR(1) DEFAULT '0' COMMENT '读取状态（1-已读/0-未读）',
  SEND_STATUS CHAR(1) DEFAULT '0' COMMENT '发送状态（1-成功/0-失败）',
  `Crt_Stff_ID` INT(11) DEFAULT NULL COMMENT '创建人',
  `Crt_Tm` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `Mod_Stff_ID` INT(11) DEFAULT NULL COMMENT '修改人',
  `Mod_Tm` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  `DEL_LBL` CHAR(1) DEFAULT '0' COMMENT '删除标志',
  PRIMARY KEY (`ID`)
) ENGINE=INNODB AUTO_INCREMENT=47 DEFAULT CHARSET=UTF8MB4;

SET FOREIGN_KEY_CHECKS = 1;

DROP TABLE IF EXISTS DATA_DIC_TBL;
CREATE TABLE DATA_DIC_TBL (
